---
title: Home
nav_order: 1
---

# Armando Ortez  
**Bioengineering (B.S.) · Applied Epidemiology (Minor)**  
Temple University

Hi — I’m Armando Ortez. I’m a bioengineering student focused on translational research at the intersection of **biomaterials, mechanobiology, and public health**. I’m building skills in experimental design, data analysis, and scientific communication as I prepare for a career in **medicine and biomedical research**.

I am currently enrolled in **Fundamentals of Geographic Information Systems (GIS)**, where I am developing skills in spatial analysis, mapping, and data visualization. This website was created as part of that course to document and showcase applied GIS work alongside my broader academic and research efforts.

This site is **preliminary** and will continue to grow into a more **robust academic and professional portfolio**.

---

## Quick links
Use the pages below to explore my work:

- **👤 About Me** → [About Me](about.md)  
- **🧪 Projects** → [Projects](projects.md)  
- **📊 Research Posters** → [Research Posters](posters.md)  
- **📝 Technical Writing** → [Technical Writing](writing.md)  
- **📑 Final Lab Report** → [Final Lab Report](final-lab-report.md)  
- **📚 Resources** → [Resources](resources.md)  
- **✉️ Contact** → [Contact](contact.md)

---

## What you’ll find here
- Research projects and engineering design work  
- Research posters presented at academic conferences  
- Technical writing papers focused on biomedical and public health topics  
- GIS maps, spatial analyses, and data visualizations  
- A comprehensive **final lab report** synthesizing course methods and results  
- Curated resources for learning and reference  
- Ongoing updates as my work and skills continue to develop

